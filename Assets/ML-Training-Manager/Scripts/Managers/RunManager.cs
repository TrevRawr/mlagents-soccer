    ...
    IEnumerator DoTrainingCo()
    {
        ...
        while (configIncrement <= MaxConfigIncrement)
        {
            ...
            for (currentRun = 0; currentRun < RunsPerConfiguration; currentRun++)
            {
                ...
                var configFile = "C:/Users/\"Trevor Clelland\"/Documents/Unity/CMPT-419-ml-agents/SoccerGame/MLSoccer/config" + "/" + RunSetName + "-inc" + configIncrement + ".yaml";
                myArguments.Add(configFile);
                myArguments.Add("--env=" + UnityOutputExeName); //+ tensorFlowConfig.UnityOutputDirectory + "/" +
                ...
                currentProcess = CommandLineRunner.StartCommandLine(tensorFlowConfig.MlAgentsRootDirectory, "mlagents-learn", myArguments.ToArray());
