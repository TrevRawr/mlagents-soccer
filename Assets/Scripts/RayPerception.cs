    ...
    public class RayPerception : MonoBehaviour
    {
            ...
            float startOffset, float endOffset)
        {
            ...
            foreach (float angle in rayAngles)
            {
                ...
                // don't use spherecasts because they collide with child objects (i.e. soccer player agent ball controllers) if the radius is too big
                //if (Physics.SphereCast(transform.position +
                //                       new Vector3(0f, startOffset, 0f), 0.3f,
                //    endPosition, out hit, rayDistance))
                if (Physics.Raycast(transform.position + new Vector3(0f, startOffset, 0f), endPosition, out hit, rayDistance))
                {
